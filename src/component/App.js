...
class App extends Component {
  ...
  async loadBreedImages() {
    ...
    let breed_images = await this.getCatsImagesByBreed(this.state.selected_breed, 6)
  ...
  render() {
      return (
        <div className="container">
            ...
            <div className="row">                 
                <div className="mycatlist">
                {this.state.images.map((image) => <div className="col-xs-12 col-sm-4"><img className="cat-image" alt="" src={image.url}></img></div>)}
                </div>
            </div>   
        </div>
      );
